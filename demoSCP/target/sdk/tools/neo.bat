...
set BASE_PATH=%~dp0
set PARAM_COMPACT_OUTPUT=-output
set PARAM_LONG_OUTPUT=--output
echo %*| FINDSTR /C:%PARAM_LONG_OUTPUT% >NUL || (echo %*| FINDSTR /C:%PARAM_COMPACT_OUTPUT% >NUL || (type "%BASE_PATH%lib\logo.txt"))
...
set ARGS=-Dneo.logging.location="%BASE_PATH%log" -Dneo.base.location="%BASE_PATH%."
...
if defined HTTPS_NON_PROXY_HOSTS for /f "delims=" %%A in ("!HTTPS_NON_PROXY_HOSTS!") do set "ARGS=!ARGS! -Dhttps.nonProxyHosts="%%~A""
...
set COMMAND=java.exe !ARGS! -cp "%CP%" -Djava.system.class.loader=com.sap.jpaas.infrastructure.console.CmdJarsClassLoader com.sap.jpaas.infrastructure.console.ConsoleClient %*
