...
then
  ...
  args="$args -Dhttps.nonProxyHosts=\"$no_proxy_hosts\""
fi
...
echo ""
exec "$javaExe" $args -Dneo.logging.location="$logPath" -Dneo.base.location="$basePath" -cp "$toolcp" -Djava.system.class.loader=com.sap.jpaas.infrastructure.console.CmdJarsClassLoader com.sap.jpaas.infrastructure.console.ConsoleClient "$@"
